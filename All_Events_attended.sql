...
SELECT  ee.name, COUNT(DISTINCT(e.event_name)) as events
FROM events e
JOIN employees ee
ON ee.id = e.emp_id
GROUP BY e.emp_id, ee.name
HAVING  COUNT(DISTINCT(e.event_name)) = (SELECT COUNT(DISTINCT event_name) FROM events)
ORDER BY e.emp_id
