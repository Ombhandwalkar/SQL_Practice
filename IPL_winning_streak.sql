...
cte_meg AS(
	...
	ROW_NUMBER() OVER(PARTITION BY teams ORDER BY teams, dates) rk
	FROM cte_teams t
	JOIN ipl_results r ON r.home_team= t.teams or r.away_team= t.teams),

cte as (
	select *,
	rk - ROW_NUMBER() OVER(PARTITION BY teams ORDER BY teams, dates) AS date_diff
	FROM cte_meg
	WHERE result= teams),

last_cte as (
	select *, COUNT(1) OVER(PARTITION BY teams, date_diff ORDER BY teams, dates RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING )as max_streak
	from cte)

SELECT teams, MAX(max_streak) AS streak
FROM last_cte
GROUP BY teams
