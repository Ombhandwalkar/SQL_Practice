...
FROM(
	SELECT c.name, o.*, RANK() OVER(PARTITION BY customer_id ORDER BY order_date DESC) AS rk
	FROM orders o
...
C2024 AS(
SELECT customer_id, COUNT(EXTRACT(MONTH FROM order_date))AS unique_monthly_users
FROM orders 
...
FROM customers c
JOIN C2024 c24 ON c.id=c24.customer_id
JOIN C2023 C23 ON c.id=c23.customer_id
WHERE c.id NOT IN (SELECT customer_id FROM C2025)


--- Solution-2
WITH CTE AS(
SELECT customer_id, c.name AS customer ,EXTRACT(YEAR FROM order_date)AS order_year, COUNT(DISTINCT EXTRACT(MONTH FROM order_date)) AS unique_monthly_orders
FROM orders o
