...
WITH CTE AS (
SELECT p.*, u.session_starttime, u.session_endtime,
EXTRACT('epoch' FROM (u.session_endtime - u.session_starttime)) as total_time
FROM post_views p
JOIN user_sessions u
ON p.session_id=u.session_id)

SELECT post_id,
	SUM((perc_viewed/100)* total_time) AS total_sec
FROM CTE
GROUP BY post_id
