...
WITH CTE AS (
	SELECT EXTRACT(MONTH FROM dates) AS months, SUM(cases_reported) AS monthly_cases
	FROM covid_cases
	GROUP BY months		
),
FINAL_CTE AS(
	SELECT *,
		SUM(monthly_cases) OVER(ORDER BY months)as total_cases
	FROM CTE)
SELECT *,
	CASE WHEN months > 1 THEN CAST(ROUND((monthly_cases / LAG(total_cases) OVER(ORDER BY months))*100,2)AS VARCHAR) ELSE '-' END AS monthly_percentage
FROM FINAL_CTE
