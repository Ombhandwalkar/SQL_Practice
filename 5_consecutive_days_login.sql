...
WITH CTE AS(
	SELECT *,
			login_date - CAST(RANK() OVER(PARTITION BY user_id ORDER BY user_id,login_date)AS INT) AS date_group
	FROM user_login
			
)
SELECT user_id, date_group, MIN(login_date) AS start_date, MAX(login_date) AS end_date,
	(MAX(login_date)- MIN(login_date))+1 AS consecutive_day
FROM CTE
